...
open class Chart: UIControl {
    ...
    override open func draw(_ rect: CGRect) {
        ...
        aPath.close()
        #colorLiteral(red: 1, green: 0.231372549, blue: 0.1882352941, alpha: 1).set()
        aPath.stroke()
    ...
    fileprivate func drawLine(_ xValues: [Double], yValues: [Double], seriesIndex: Int) {
        ...
        path.move(to: CGPoint(x: CGFloat(xValues.first! + 25), y: CGFloat(yValues.first!)))
        for i in 1..<yValues.count {
            let y = yValues[i]
            path.addLine(to: CGPoint(x: CGFloat(xValues[i] + 25), y: CGFloat(y)))
        }
    ...
    fileprivate func drawLabelsAndGridOnYAxis() {
        ...
        var labels: [Double]
        if yLabels == nil {
            let first = (min.y + max.y) * (1/5)
            let two = (min.y + max.y) * (2/5)
            let three = (min.y + max.y) * (3/5)
            let four = (min.y + max.y) * (4/5)
            let five = (min.y + max.y) * (5/5)
            labels = [min.y, first, two, three, four, five, max.y]
